...
from .models import Transaction,MoneyTransfer ,DEPOSIT,WITHDRAW,LOAN,LOAN_PAID,MONEY_TRANSFER
...
def send_transaction_email(user, amount, subject, template):
    ...
    send_mail.attach_alternative(message)
    send_mail.send()
...
class MoneyTransferView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            receiver = UserAccount.objects.get(account_no=receiver_no)
            if receiver:
                account = self.request.user.account
